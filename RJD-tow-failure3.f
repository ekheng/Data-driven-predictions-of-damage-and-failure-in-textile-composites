...
!           CALL XPLB_EXIT
	  ...
	  IF (STRESS(1).lt.(-1.0D0*sigcr11)) THEN
!	  write(*,*) 'Element in compression', CMNAME	
        STATEV(200)=1.0d0
!	  CALL XPLB_EXIT
	  ENDIF
...
!CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC	  
     ...
     1	                              + (STRESS(6)/sigcr23)**2
		 ...
		 IF ((HASHIN2.ge.1.0d0).and.(STRESS(2).gt.0.0d0)) THEN
            ...
            IF (h2.ge.(0.95*BZ22)) THEN
		GIc2=0.5D0*h2*(STATEV(184)**2)/E22c
		END IF
	    IF (h2.ge.(0.95*BZ12)) THEN
		GIc12=0.5D0*h2*(STATEV(185)**2)/G12c
	    END IF
            IF (h2.ge.(0.95*BZ23)) THEN
		GIc23=0.5D0*h2*(STATEV(186)**2)/G23c
